...
from kinematic_fit_v2 import KinematicFit
...
def main():
    ...
    constraints = []
    cnstr1 = e1**2 + e2**2 + einv**2 - sqrt_s
    #cnstr2 = e1 + e2 + einv - sqrt_s
    # cnstr3 = ...
    #constraints.append(cnstr1)
    constraints.append(cnstr1)

    """ declare kinematic fit object """
    kin_fit = KinematicFit(measurements, unmeasurables, constraints)

    kin_fit.print_constraints()
    kin_fit.print_constraints_derivatives()
    ...
    for iev in range(10):
        ...
        V = np.array([[sigma1**2, 1.e-6],
                     [1.e-6, sigma2**2]])

        # starting values
        eta0 = y
        chi0 = np.array([[einv_reco]])

        options = (maxloops, maxDeltaChiSq, weight)
        (eta, chi, V, chisqrd_last, i) = kin_fit.results(y, V, eta0, chi0, options)
